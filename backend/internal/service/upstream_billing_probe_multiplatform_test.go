...
func TestUpstreamBillingProbeUnknownPlatformCustomRelayPersistsSnapshot(t *testing.T) {
	...
	require.Equal(t, UpstreamBillingProbeStatusOK, snapshot.Status)
	require.Len(t, upstream.requests, 2)
	require.Equal(t, "https://relay.future.example/v1/sub2api/billing", upstream.requests[0].URL.String())
	require.Equal(t, "https://relay.future.example/v1/usage", upstream.requests[1].URL.String())
	require.Equal(t, "Bearer sk-future-relay", upstream.requests[0].Header.Get("Authorization"))
	require.Equal(t, HTTPUpstreamProfileDefault, HTTPUpstreamProfileFromContext(upstream.requests[0].Context()))
