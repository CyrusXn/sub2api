...
func RegisterGatewayRoutes(
	...
	gemini.Use(middleware.APIKeyAuthWithSubscriptionGoogle(apiKeyService, subscriptionService, cfg, balanceNotifyService))
	...
	// Codex 自定义 provider 固定从根 /models 发现模型；新版客户端可能不再携带 client_version。
	r.GET("/models", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), requireGroupAnthropic, codexModelsHandler)
	...
	antigravityV1Beta.Use(middleware.APIKeyAuthWithSubscriptionGoogle(apiKeyService, subscriptionService, cfg, balanceNotifyService))
