...
func (h *AccountHandler) SetUpstreamSiteCredentialService(credentials *service.UpstreamSiteCredentialService) {
	h.upstreamSiteCredentials = credentials
}
...
// NullableFloat64Field 保留 JSON 字段是否出现，支持用 null 清除已保存的手动倍率。
type NullableFloat64Field struct {
	Set   bool
	Value *float64
}

func (f *NullableFloat64Field) UnmarshalJSON(raw []byte) error {
	f.Set = true
...
type UpdateAccountRequest struct {
	...
	UpstreamBillingManualRateMultiplier NullableFloat64Field                    `json:"upstream_billing_manual_rate_multiplier"`
	UpstreamSubscriptionRate            *service.UpstreamSubscriptionRateConfig `json:"upstream_subscription_rate"`
	ConfirmMixedChannelRisk             *bool                                   `json:"confirm_mixed_channel_risk"` // 用户确认混合渠道风险
}
...
func (h *AccountHandler) Update(c *gin.Context) {
	...
	account, err := h.adminService.UpdateAccount(c.Request.Context(), accountID, &service.UpdateAccountInput{
		...
		UpstreamBillingManualRateMultiplierSet: req.UpstreamBillingManualRateMultiplier.Set,
		UpstreamBillingManualRateMultiplier:    req.UpstreamBillingManualRateMultiplier.Value,
		UpstreamSubscriptionRate:               req.UpstreamSubscriptionRate,
		SkipMixedChannelCheck:                  skipCheck,
	})
	if err != nil {
		// 检查是否为混合渠道错误
