...
func TestAccountFromServiceShallow_RedactsSensitiveCredentials(t *testing.T) {
	src := &service.Account{
		...
		Credentials: map[string]any{
			...
			"web_login_password": "compat-login-secret",
	...
	require.NotContains(t, got.Credentials, "login_password")
	require.NotContains(t, got.Credentials, "web_login_password")
	// 非敏感键保留
	...
	require.True(t, got.CredentialsStatus["has_login_password"])
	require.True(t, got.CredentialsStatus["has_web_login_password"])
	...
	require.NotContains(t, string(raw), "login-secret")
	require.NotContains(t, string(raw), "compat-login-secret")
	// 状态标识应序列化进 JSON
