...
func TestUsageLogRepositoryUsageAggregatesFilterNativeCompactionV2(t *testing.T) {
	...
	t.Run("stats", func(t *testing.T) {
		db, mock := newSQLMock(t)
		repo := &usageLogRepository{sql: db}
		// 本站聚合查询在 usage_logs 后带上游成本 LEFT JOIN，官方原正则的 \s+ 无法匹配，放宽为 .*（与同文件其余子测试一致）。
		mock.ExpectQuery("(?s)FROM usage_logs.*WHERE native_compaction_v2 = \\$1.*GROUP BY GROUPING SETS").
			WithArgs(true).
...
func TestUsageLogRepositoryGetStatsWithFiltersAdminViewUsesSettledValues(t *testing.T) {
	...
	mock.ExpectQuery("(?s)FROM usage_logs.*COALESCE\\(SUM\\(actual_cost\\), 0\\) AS actual_cost.*GROUP BY GROUPING SETS").
		WillReturnRows(sqlmock.NewRows([]string{
			"inbound_grouped",
			"upstream_grouped",
			"inbound_endpoint",
...
func TestAppendUsageLogSharedListConditionsPreservesLegacySemantics(t *testing.T) {
	filters := usagestats.UsageLogFilters{
		UserIDs:                []int64{2, 3},
		UpstreamSiteAccountIDs: []int64{7, 8},
		RequestTypes:           []int16{int16(service.RequestTypeSync), int16(service.RequestTypeStream)},
		BillingModes:           []string{string(service.BillingModeToken), string(service.BillingModeImage)},
	}

	conditions, args := appendUsageLogSharedListConditions(nil, nil, filters, "ul")
	where := strings.Join(conditions, " AND ")

	require.Contains(t, where, "ul.user_id = ANY($1)")
	require.Contains(t, where, "ul.account_id = ANY($2)")
	require.Contains(t, where, "ul.request_type = $3")
	require.Contains(t, where, "ul.request_type = $4")
	require.Contains(t, where, "ul.stream = FALSE")
	require.Contains(t, where, "ul.stream = TRUE")
	require.Contains(t, where, "ul.billing_mode = $5")
	require.Contains(t, where, "ul.billing_mode = $6")
	require.Contains(t, where, "COALESCE(ul.image_count, 0) <= 0")
	require.Contains(t, where, "COALESCE(ul.image_count, 0) > 0")
