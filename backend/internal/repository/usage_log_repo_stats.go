...
// GetAdminBatchAPIKeyUsageStats 按管理端已结算口径聚合密钥消费，保持用户端展示口径不变。
func (r *usageLogRepository) GetAdminBatchAPIKeyUsageStats(ctx context.Context, apiKeyIDs []int64, startTime, endTime time.Time) (map[int64]*BatchAPIKeyUsageStats, error) {
	...
	query := fmt.Sprintf(`
		SELECT
			ul.api_key_id,
			COALESCE(SUM(ul.actual_cost * %s) FILTER (WHERE ul.created_at >= $2 AND ul.created_at < $3), 0) AS total_cost,
			COALESCE(SUM(ul.actual_cost * %s) FILTER (WHERE ul.created_at >= $4), 0) AS today_cost
		FROM usage_logs ul
	...
	`, adminUsageMultiplierSQLExpr, adminUsageMultiplierSQLExpr)
	today := timezone.Today()
	rows, err := r.sql.QueryContext(ctx, query, pq.Array(normalizedAPIKeyIDs), startTime, endTime, today)
	if err != nil {
		return nil, err
...
func (r *usageLogRepository) GetStatsWithFilters(ctx context.Context, filters UsageLogFilters) (*UsageStats, error) {
	...
	stats := &UsageStats{}
	var totalAccountCost, totalUpstreamCost float64
	...
	for rows.Next() {
		...
		case inboundGrouped == 1 && upstreamGrouped == 1:
			...
			totalUpstreamCost = upstreamCost
	...
	stats.TotalUpstreamCost = &totalUpstreamCost
	stats.TotalTokens = stats.TotalInputTokens + stats.TotalOutputTokens + stats.TotalCacheTokens
