...
func TestDashboardAggregationRepositoryCleanupUsageLogsNonPartitionedInvalidatesEachBatchAndSyncs(t *testing.T) {
	...
	expectDashboardBusinessFinalization(mock, cutoff)
...
// 自动清理必须先固化经营汇总，后续分组日桶失效流程才能安全删除明细。
func expectDashboardBusinessFinalization(mock sqlmock.Sqlmock, cutoff time.Time) {
	mock.ExpectExec(`UPDATE dashboard_business_daily`).
		WithArgs(cutoff, "Asia/Shanghai").
		WillReturnResult(sqlmock.NewResult(0, 1))
}
