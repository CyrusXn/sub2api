...
func TestMigration232CreatesGroupUsageRollups(t *testing.T) {
	content, err := FS.ReadFile("232_group_usage_daily_rollups.sql")
...
func TestMigration232InvalidatesClosedBucketsWhenUsageLogsChange(t *testing.T) {
	content, err := FS.ReadFile("232_group_usage_daily_rollups.sql")
	require.NoError(t, err)
...
func TestMigration233TracksConfiguredTimezone(t *testing.T) {
	content, err := FS.ReadFile("233_group_usage_rollup_timezone.sql")
	require.NoError(t, err)
