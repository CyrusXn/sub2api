<template>
  <AppLayout>
      ...
      <template #table>
        ...
        >
          ...
          <template #cell-groups="{ row }">
            <AccountGroupsCell :groups="accountGroupsForRow(row)" :max-display="8" />
          </template>
    ...
    <AccountActionMenu :show="menu.show" :account="menu.acc" :anchor-rect="menu.anchorRect" @close="menu.show = false" @delete="handleDelete" @test="handleTest" @stats="handleViewStats" @schedule="handleSchedule" @duplicate="handleDuplicateAccount" @reauth="handleReAuth" @refresh-token="handleRefresh" @recover-state="handleRecoverState" @reset-quota="handleResetQuota" @set-privacy="handleSetPrivacy" @create-spark-shadow="handleCreateSparkShadow" />
...
// 紧凑账号列表只返回 group_ids，展示时按目录映射出分组名称。
...
type AccountLoadOptions = {
  ...
  refreshUpstreamBilling?: boolean
  forceUpstreamBillingRefresh?: boolean
}
...
const reload = async (options: AccountLoadOptions = {}) => {
  syncAccountListDerivedParams()
  hasPendingListSync.value = false
  resetAutoRefreshCache()
  pendingTodayStatsRefresh.value = false
...
const loadAccountDetails = async (account: Pick<AccountListItem, 'id'>): Promise<Account | null> => {
  ...
  try {
    ...
    appStore.showError(error instanceof Error ? error.message : extractApiErrorMessage(error, t('common.error')))
    return null
  } finally {
