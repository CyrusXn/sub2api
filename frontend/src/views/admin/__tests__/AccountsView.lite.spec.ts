...
vi.mock('@/api/admin', () => ({
  adminAPI: {
    ...
    tablePreferences: { get: async () => ({ exists: false }), save: async () => undefined },
...
describe('admin AccountsView lite account list', () => {
  ...
  it('keeps the action menu open during internal scrolling but closes it on table scrolling', async () => {
    ...
    // 定制版更多操作使用图标按钮，以稳定操作标识定位。
    const trigger = wrapper.get('[data-test="account-action-more"]')
  ...
  it('loads the full account by id before opening edit, test, and stats actions', async () => {
    ...
    const editButton = wrapper.find('[data-test="account-action-edit"]')
